...
ArrDict{K, V}() where{K, V} = ArrDict{K, V}(Dict{K, V}(), Vector{V}() )
ArrDict{K, V}(prealloc_size) where{K, V} = ArrDict{K, V}(Dict{K, V}(), Vector{V}(undef, prealloc_size))
...
function setindex!(d::ArrDict{K, V}, value :: V, key :: K) :: V where {K, V}
    if !(key in keys(d.key_to_index))
        write_index =  length(d.key_to_index) + 1
        if write_index <= length(d.values)
            d.values[write_index] = value
        else
            push!(d.values, value)
        end
        d.key_to_index[key] = write_index
    else
        index = d.key_to_index[key]
        @inbounds d.values[index] = value
    end
    return value
end

function getindex(d::ArrDict{K, V}, key :: K) :: V where {K, V}
    if !(key in keys(d.key_to_index))
        throw(KeyError(key))
    end
    @inbounds return d.values[d.key_to_index[key]]
end

function keys(d :: ArrDict{K, V}) where {K, V}
    return keys(d.key_to_index)
end

# for testing
function digest(str, p1=11, p2=53)
    munch_name(Vector{UInt8}("$(str);"), 1, p1, p2)[1]
end


@inline function munch_name(buffy, i, p1=11, p2=53)
    # p1 and p2 are params that adjust the digesting
    @inbounds begin
        # creates a shitty digest of the name encoded starting at buffy[i] until the next semi colon from position i 
        # returns digest and index to start parsing number at
...
@inline function munch_decimal(buffy, i :: I) :: Tuple{Int32, I} where I
    @inbounds begin
...
function process_chunk!(result_dict::ArrDict{UInt64, NTuple{4, Int32}}, buffy)
    rows = 0
    bytes_read = 1
    
    @inbounds while bytes_read + 1 < length(buffy)
...
function main()
    ...
    rows = 0
    result_dict = ArrDict{UInt64, NTuple{4, Int32}}(1<<15)
    tuple_arr = Vector{NTuple{4, Int32}}()
    open(PATH, "r") do file
        while true
