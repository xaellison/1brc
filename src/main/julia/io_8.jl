...
function process_chunk!(result_dict, buffer)
    ...
    while bytes_read + 1 < length(buffer)
        ...
        digest, bytes_read = parse_name_digest(buffer, bytes_read)
...
expected_max = Dict(map(kv -> (name_digest(kv[1]), Int(kv[2]*10)), zip(maxima.name, maxima.x_maximum)))

actual_max = Dict(k => v[2] for (k,v) in d)
